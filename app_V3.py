...
.dl-pdf .stDownloadButton > button {
  ...
  font-weight: 700 !important;
  font-size: 0.87rem !important;
  box-shadow: 0 4px 16px rgba(220,38,38,0.28) !important;
  transition: all .2s !important;
}
...
# Dynamic morale templates — {role} is substituted at render time
MORALE_TEMPLATES = [
    ("🚀", "As a {role}, your foundation is solid — and now your CV proves it. Go own that interview!"),
    ("🏆", "Every great {role} started where you are right now. This CV is your next step up."),
    ("⚡", "The best {role} in the room walks in prepared. That's exactly what you are today."),
    ("🎯", "Hiring managers looking for a {role} are about to see your full potential — unfiltered."),
    ("💎", "A {role} of your calibre deserves a CV that commands attention. Now you have one."),
    ("🔥", "Bots beaten, keywords matched — now go show them why you're the {role} they need."),
    ("🌟", "This isn't just a resume. It's a {role}'s story, told with precision and impact."),
    ("🦁", "The role of {role} doesn't choose anyone. You've earned your shot — go take it."),
]
...
def render_ats(score, color, bar_color, gap):
    ...
    <div class="ats-wrap">
      ...
      <div class="ats-gap">
        <strong>What's missing for this role?</strong><br>{gap}
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if not st.session_state.payment_verified:
        ...
        if st.session_state.downloaded:
            if st.session_state.morale_msg is None:
                ...
                st.session_state.morale_msg = (icon, tpl.format(role=role))
